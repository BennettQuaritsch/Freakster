...
import type { SongCardImage } from '$lib/types/song-card';
...
export async function buildCardsPdf(
	...
	for (let index = 0; index < images.length; index += 1) {
		...
		const bytes = new Uint8Array(await images[index].blob.arrayBuffer());
	...
	const safeBytes = new Uint8Array(Array.from(pdfBytes));
